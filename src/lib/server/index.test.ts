...
import { createBraintreeGateway, generateClientToken, buildTransactionRequest, createTransaction, parseWebhook } from './index.js'
...
describe('createBraintreeGateway', () => {
	...
	it('constructs a gateway for a valid environment', () => {
		const gw = createBraintreeGateway({
			environment: 'sandbox',
			merchantId: 'm',
			publicKey: 'p',
			privateKey: 'k'
...
describe('generateClientToken', () => {
	it('unwraps the clientToken string', async () => {
		const gateway = {
			clientToken: { generate: vi.fn().mockResolvedValue({ clientToken: 'ct_abc' }) }
		}
...
describe('createTransaction', () => {
	it('calls gateway.transaction.sale with the built request', async () => {
		...
		await createTransaction({ transaction: { sale } } as never, {
			amount: '10',
			nonce: 'n',
			deviceData: 'dd'
		})
		expect(sale).toHaveBeenCalledWith(expect.objectContaining({ amount: '10', paymentMethodNonce: 'n', deviceData: 'dd' }))
	})
})
...
describe('parseWebhook', () => {
	it('delegates to webhookNotification.parse(signature, payload)', async () => {
		...
		await parseWebhook({ webhookNotification: { parse } } as never, {
			signature: 'sig',
			payload: 'pl'
		})
