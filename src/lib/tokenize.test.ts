...
describe('tokenizeCard', () => {
	it('passes options through and returns nonce+details', async () => {
		const hf = {
			tokenize: vi.fn().mockResolvedValue({ nonce: 'n1', details: { bin: '411111' } })
		}
...
describe('verifyCard', () => {
	it('coerces amount, auto-continues the lookup, returns the verified nonce', async () => {
		...
		const r = await verifyCard({ verifyCard: verifyCardFn } as never, {
			amount: 20,
			nonce: 'n1',
			bin: '411111'
		})
		expect(verifyCardFn).toHaveBeenCalledWith(expect.objectContaining({ amount: '20', nonce: 'n1', bin: '411111' }))
...
describe('tokenize (combined)', () => {
	...
	const tds = (result: unknown) => ({
		verifyCard: vi.fn().mockImplementation(opts => {
			opts.onLookupComplete?.({}, () => {})
